...
const plans = {
  pro: { title: "DevForge Pro", description: "Annual access to every course and project.", price: 149, features: ["All current courses", "Certificates", "Progress tracking", "Community access"] },
  lifetime: { title: "DevForge Lifetime", description: "Permanent access, including future courses.", price: 399, features: ["Everything in Pro", "Lifetime access", "Future courses", "Premium projects"] },
};
export function generateStaticParams() { return [...courses.map(({ slug }) => ({ course: slug })), { course: "pro" }, { course: "lifetime" }]; }
...
export default async function CheckoutPage({ params }: PageProps) {
  ...
  const item = plan ?? { title: course!.title, description: course!.shortDescription, price: course!.price, features: [`${course!.duration} guided path`, `${course!.projects} portfolio projects`, "Certificate of completion", "Lifetime course access"] };
