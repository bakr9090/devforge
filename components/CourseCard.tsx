...
export function CourseCard({ course }: { course: Course }) {
  return (
    <article className="course-card surface card-lift group flex h-full flex-col">
      <CourseVisual course={course} />
      <div className="flex flex-1 flex-col p-6">
        <div className="mb-4 flex items-center justify-between"><span className="eyebrow text-[var(--muted)]">{course.category}</span><span className="course-price text-xl font-bold">${course.price}</span></div>
        ...
        <Link href={`/courses/${course.slug}`} className="course-cta focus-ring flex items-center justify-between font-bold transition-colors group-hover:text-[var(--mint)]">View course <ArrowUpRight size={18} /></Link>
      </div>
    </article>
  );
}
